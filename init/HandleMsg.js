...
module.exports = HandleMsg = async (aruga, message) => {
        ...
        function isStickerMsg(id){
            ...
            for (let i of stickerspam){
                if(i.id === id){
                    if (i.msg >= 7) {
                        found === true 
                        aruga.reply(from, '*[ANTI STICKER SPAM]*\nTienes Span De Stickers, El Bot De Pateara...', message.id).then(() => {
                            ...
                            if (found !== false) {
                                ...
                                const result = '✅ DB Sticker Spam Se Ha Restabecido..'
                                console.log(stickerspam[found])
                                fs.writeFileSync('./lib/helper/stickerspam.json',JSON.stringify(stickerspam));
                                aruga.sendText(from, result)
                            } else {
                                    aruga.reply(from, `${monospace(`Lo Siento Este Numero No Existe.`)}`, id)
                            }
                        })
                        return true;
                    }else{
	...
	// Filter Banned People
        ...
        case 'ownerbot':
            ...
            break
        case 'join':
            if (args.length == 0) return aruga.reply(from, `Si desea invitar al bot al grupo por favor invite \n ${prefix}Unete [link group]`, id)
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Soy El Bot Para Conocer Los Comandos Escribe ${prefix}menu`)
                      })
            } else {
                let cgrup = await aruga.getAllGroups()
                if (cgrup.length > groupLimit) return aruga.reply(from, `Lo siento, el grupo de este bot está lleno\nEl Maximo es: ${groupLimit}`, id)
                if (cgrup.size < memberLimit) return aruga.reply(from, `Lo sentimos, Bot no se unirá si los miembros del grupo no superan ${memberLimit} Personas`, id)
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () =>{
	...
	case 'stimg':
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                await aruga.sendFile(from, imageBase64, 'imgsticker.jpg', 'convierte la pegatina en imagen!', id)
                .then(() => {
                    console.log(`Sticker a Imagen Procesado en ${processTime(t, moment())} Seconds`)
                })
        } else if (!quotedMsg) return aruga.reply(from, `Formato incorrecto en la pegatina que quieres usar como imagen.`, id)
        break
    ...
    case 'logopornhub':
            ...
            if (argz.length >= 2) {
                aruga.reply(from, `Se Paciente...`, id)
	...
	case 'coolteks':
	case 'cooltext':
            if (args.length == 0) return aruga.reply(from, `Para hacer que CoolText texto Funcione en imágenes use ${prefix}cooltext teks\n\nContoh: ${prefix}cooltext bot-code`, id)
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                .then(() => {
                    ...
                    console.log(`Sticker Procesado enr ${processTime(t, moment())} Second`)
                })
            } else if (args[0] === 'nobg') {
                ...
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ...
                    : aruga.reply(from, 'Aqui Esta Tu Sticker')).then(() => console.log(`Sticker Procesado en ${processTime(t, moment())} Second`))
            } else {
                await aruga.reply(from, `No Fotos Usar ${prefix}sticker\n\n\nEnviar Fotos Con Subtitulos\n${prefix}sticker <normal>\n${prefix}sticker nobg <tanpa background>\n\nO Enviar Msj Con\n${prefix}sticker <link>`, id)
            }
            break
            case 'antisticker':
            case 'antistiker':
                    if (!isGroupMsg) return aruga.reply(from, 'Lo sentimos, ¡este comando solo se puede usar dentro de grupos!', id)
                    if (!isGroupAdmins) return aruga.reply(from, 'Falló, este comando solo puede ser utilizado por administradores de grupo.', id)
                    if (!isBotGroupAdmins) return aruga.reply(from, 'Primero hazme el administrador del grupo :)', id)
                    if (args[0] == 'on') {
                        var cek = antisticker.includes(chatId);
                        if(cek){
                            return aruga.reply(from, '*El detector de pegatinas antispam * ya está activo en este grupo', id) //if number already exists on database
                        } else {
                            antisticker.push(chatId)
                            fs.writeFileSync('./lib/helper/antisticker.json', JSON.stringify(antisticker))
                            aruga.reply(from, '*[Anti Sticker SPAM]* ha sido activado \n¡Cada miembro del grupo cuyos Stickers de spam sea más de 7 será expulsado por el bot!', id)
                        }
                    } else if (args[0] == 'off') {
                        ...
                        aruga.reply(from, `Elija On / off\n\n*[Anti Sticker SPAM]*\n¡Todos los miembros del grupo que Hagan spam Con Stickers serán expulsados ​​por el bot!`, id)
                    }
                    break
                    case 'antilink':
                    if (!isGroupMsg) return aruga.reply(from, 'Lo sentimos, ¡este comando solo se puede usar dentro de grupos!', id)
                    if (!isGroupAdmins) return aruga.reply(from, 'Falló, este comando solo puede ser utilizado por administradores de grupo.', id)
                    if (!isBotGroupAdmins) return aruga.reply(from, 'Primero Hazme *ADMINISTRADOR* Del Grupo:)', id)
                    if (args[0] == 'on') {
                        var cek = antilink.includes(chatId);
                        if(cek){
                            return aruga.reply(from, '*Anti Group Link Detector* Activo En Este Grupo', id) //if number already exists on database
                        } else {
                            antilink.push(chatId)
                            fs.writeFileSync('./lib/helper/antilink.json', JSON.stringify(antilink))
                            aruga.reply(from, '*[Anti Group Link]* ha sido activado ¡Cada miembro del grupo que envíe un mensaje que contenga el enlace del grupo será expulsado por el bot!', id)
                        }
                    } else if (args[0] == 'off') {
                        var cek = antilink.includes(chatId);
                        if(!cek){
                        ...
                        aruga.reply(from, `Elija on / off\n\n*[Anti Group Link]*\n¡Cada miembro del grupo que publique un mensaje que contenga el enlace del grupo será expulsado por el bot!`, id)
                    }
                    break  
	case 'brainly':
            ...
            if (args.length >= 2){
			    ...
			    aruga.sendText(from, 'Selesai ✅, donasi kesini ya paypal.me/TheSploit | Pulsa : 09646576676776')
                        } else {
        ...
        case 'stikergif':
            ...
            break
        case 'sock5':
        const misproxis = await axios('https://api.proxyscrape.com/?request=displayproxies&proxytype=socks5')
        if(misproxis){
            aruga.reply(from, `Proxys Obtenidos\n\n${misproxis.data}`, id)
        }else{
            aruga.reply(from, `No Se Obtuvieron Resultados`)
        }
        break
        case 'infovps':
        case 'stikergiphy':
        case 'stickergiphy':
            if (args.length !== 1) return aruga.reply(from, `Lo sentimos, el formato del mensaje es incorrecto. Escriba el mensaje con ${prefix}stickergiphy <link_giphy>`, id)
            ...
            if (isGiphy) {
                ...
                aruga.sendGiphyAsSticker(from, smallGifUrl).then(() => {
                    aruga.reply(from, 'Aqui Esta Tu Sticker')
                    console.log(`Sticker Procesado en ${processTime(t, moment())} Second`)
                ...
                .then(() => {
                    aruga.reply(from, 'Aqui Est Tu Sticker')
                    console.log(`Sticker Procesado en ${processTime(t, moment())} Second`)
                })
                .catch(() => {
                    aruga.reply(from, `Algo Salio Mal!`, id)
                })
            } else {
    ...
    case 'qrcode':
      ...
      break			
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                aruga.sendFile(from, ImageBase64, 'image.png', '', null, true)
                    ...
                    .catch(() => {
                        aruga.reply(from, 'A Ocurrido Un Error!')
                    })
            } else {
        ...
        case 'quotemaker':
            ...
            if (qmaker.length >= 3) {
                ...
                try {
                    ...
                    aruga.reply('Bueno, el proceso falló, hermano, el contenido aún es correcto.?..', id)
                }
            } else {
        ...
        case 'nulis':
            ...
            .catch(() => {
                aruga.reply(from, 'A Ocurrido Un Error!', id)
            })
            break
        ...
        case 'listsurah':
            try {
                ...
                .then((response) => {
                    let hehex = '╔══✪〘 List Comando Bot - Code 〙✪══\n'
                    for (let i = 0; i < response.data.data.length; i++) {
                        hehex += '╠➥ '
                        hehex += response.data.data[i].name.transliteration.id.toLowerCase() + '\n'
                            }
                        hehex += '╚═〘 *Cod3 Err0r  B O T* 〙'
            ...
            break
        case 'infosurah':
            if (args.length == 0) return aruga.reply(from, `*_${prefix}infosurah <nama surah>_*\nMuestra información completa sobre una determinada Funcion. Ejemplo de uso: ${prefix}infosurah al-baqarah`, message.id)
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        //Media
        case 'ytmp3':
            if (args.length == 0) return aruga.reply(from, `Para descargar canciones de youtube\Escribe: ${prefix}ytmp3 [link_yt]`, id)
            ...
            break
        case 'ytmp4':
            if (args.length == 0) return aruga.reply(from, `Para descargar Canciones de youtube\Escriba: ${prefix}ytmp3 [link_yt]`, id)
	...
	case 'dewabatch':
        ...
        case 'stalkig':
            ...
            break
        case 'wiki':
            if (args.length == 0) return aruga.reply(from, `Para encontrar una palabra de wikipedia\Escribe: ${prefix}wiki [kata]`, id)
    ...
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
	    ...
	    case 'add':
            if (!isGroupMsg) return aruga.reply(from, 'Lo sentimos, ¡este comando solo se puede usar dentro de grupos!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Falló, este comando solo puede ser utilizado por administradores de grupo.', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Error, agregue el bot como administrador de grupo.', id)
	        if (args.length !== 1) return aruga.reply(from, `Usar ${prefix}add\nUtilizar: ${prefix}add <Numero>\nEjemplo: ${prefix}add 628xxx`, id)
                try {
                    await aruga.addParticipant(from,`${args[0]}@c.us`)
                } catch {
                    aruga.reply(from, 'No se puede agregar...', id)
                }
            break
        case 'kick':
            if (!isGroupMsg) return aruga.reply(from, 'Lo sentimos, ¡este comando solo se puede usar dentro de grupos!', id)
        ...
        case 'promote':
            ...
            await aruga.sendTextWithMentions(from, `Solicitud aceptada, @${mentionedJidList[0].replace('@c.us', '')} Ya Es Administrador.`)
            break
        case 'demote':
            if (!isGroupMsg) return aruga.reply(from, 'Lo sentimos, ¡este comando solo se puede usar dentro de grupos!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Falló, este comando solo puede ser utilizado por administradores de grupo.', id)
            ...
            if (mentionedJidList.length !== 1) return aruga.reply(from, 'Lo sentimos, solo se puede demostrar 1 usuario', id)
            if (!groupAdmins.includes(mentionedJidList[0])) return await aruga.reply(from, 'Lo sentimos, el usuario aún no es administrador.', id)
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, 'Lo sentimos, el formato del mensaje es incorrecto.\nNo se puede hacer una demostración de la cuenta del bot', id)
            await aruga.demoteParticipant(groupId, mentionedJidList[0])
            await aruga.sendTextWithMentions(from, `Solicitud aceptada, admin @${mentionedJidList[0].replace('@c.us', '')} removido del cargo.`)
            break
        case 'bye':
            if (!isGroupMsg) return aruga.reply(from, 'Lo sentimos, ¡este comando solo se puede usar dentro de grupos!', id)
            ...
            break
        case 'del':
            if (!isGroupAdmins) return aruga.reply(from, 'Lo sentimos, ¡este comando solo se puede usar dentro de grupos!', id)
		...
		case 'mutegrup':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Falló, este comando solo puede ser utilizado por administradores de grupo.', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Error, agregue el bot como administrador de grupo.', id)
			if (args.length !== 1) return aruga.reply(from, `Para cambiar la configuración del chat grupal para que solo el administrador pueda chatear\n\nEscribe:\n${prefix}mutegrup on\n\nO Desactiva Con ${prefix}mutegrup off `, id)
            if (args[0] == 'on') {
				aruga.setGroupToAdminsOnly(groupId, true).then(() => aruga.sendText(from, 'Se modificó correctamente para que solo el administrador pueda chatear.'))
			} else if (args[0] == 'off') {
				aruga.setGroupToAdminsOnly(groupId, false).then(() => aruga.sendText(from, '¡Cambiado exitosamente para que todos los miembros puedan chatear!'))
			} else {
		...
		case 'setprofile':
			...
			if (isMedia && type == 'image' || isQuotedImage) {
				...
				: aruga.reply(from, 'Profie Grup Cambiado Exitosamente', id))
			} else {
		...
		case 'welcome':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Falló, este comando solo puede ser utilizado por administradores de grupo.', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Error, agregue el bot como administrador de grupo.', id)
			if (args.length !== 1) return aruga.reply(from, `Para Activar El Mensaje De Bienvenida Escribe:\n${prefix}welcome on \n\NO ${prefix}welcome off Para Desactivar\n${prefix}welcome off --nonaktifkan`, id)
			if (args[0] == 'on') {
				welcome.push(chatId)
				...
				fs.writeFileSync('./settings/welcome.json', JSON.stringify(welcome))
				aruga.reply(from, 'El mensaje de bienvenida ahora está deshabilitado', id)
			} else {
				aruga.reply(from, `Para Activar El Mensaje De Bienvenida Escribe:\n${prefix}welcome on \n\NO ${prefix}welcome off Para Desactivar\n`, id)
			}
			break
        ...
        if (!isBotGroupAdmins) return aruga.reply(from, 'Error, agregue el bot como administrador de grupo.', id)
            ...
            aruga.reply(from, 'Pateando A Todo El Grupo', id)
        break

        //Owner Bot
        case 'ban':
            if (!isOwnerBot) return aruga.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
            if (args.length == 0) return aruga.reply(from, `Prohibir Que Alguin Use Comandos\n\nEscribir: \n${prefix}ban add 628xx Para Activar O\n${prefix}ban del 628xx Para Desactivar\n\ncómo prohibir rápidamente muchos tipos de grupos:\n${prefix}ban @tag @tag @tag`, id)
            if (args[0] == 'add') {
                banned.push(args[1]+'@c.us')
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                aruga.reply(from, 'Baneado Exitosamente!')
            } else
            if (args[0] == 'del') {
                let xnxx = banned.indexOf(args[1]+'@c.us')
                banned.splice(xnxx,1)
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                aruga.reply(from, 'Desbaneado Exitosamente!')
            } else {
             for (let i = 0; i < mentionedJidList.length; i++) {
                banned.push(mentionedJidList[i])
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                aruga.reply(from, 'Baneado!', id)
                }
            }
            break
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *CoD3 Err0r* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *CoD3 Err0r* 〙\n\n${msg}`)
            }
